...
export default function Root() {
      ...
      <div className="min-h-screen bg-[#0f172a] text-slate-100 font-sans selection:bg-blue-500 selection:text-white">
        ...
        <section className="px-4 py-20 text-center max-w-4xl mx-auto">
          ...
          <p className="text-lg md:text-xl text-slate-400 mb-8 leading-relaxed max-w-3xl mx-auto">
            İadeNabız, e-ticaret satıcıları için geliştirilen yapay zekâ destekli operasyon platformudur. İadelerin gerçek nedenlerini analiz edin, riskli ürünleri tespit edin ve günlük operasyonlarınızı veriye dayalı hale getirin.
          </p>
        ...
        <section className="px-4 py-20 max-w-4xl mx-auto text-center">
          ...
          <p className="text-slate-300 text-lg leading-relaxed mb-6">
            Birçok satıcı yalnızca kaç ürünün iade edildiğini görür. İadeNabız ise bu iadelerin neden gerçekleştiğini analiz eder. Beden uyumsuzluğu mu, açıklama hatası mı yoksa paketleme kaynaklı bir sorun mu olduğunu tespit eder.
          </p>
        </section>
        ...
        <section id="sss" className="px-4 py-12 max-w-4xl mx-auto">
          <h2 className="text-3xl font-bold text-center text-white mb-8">Sıkça Sorulan Sorular</h2>
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex flex-col">
      <header className="border-b border-slate-800 bg-slate-900/90 px-6 py-4 flex justify-between items-center sticky top-0 z-30 backdrop-blur-md">
        <div className="flex items-center gap-6">
          ...
          <nav className="hidden md:flex items-center gap-4 text-sm font-medium">
            <button 
              ...
              className={`px-3 py-1.5 rounded-lg transition-colors ${view === 'dashboard' ? 'bg-slate-800 text-white' : 'text-slate-400 hover:text-white'}`}
            >
              Genel Bakış
            </button>
            <button 
              onClick={() => setView('products')} 
              className={`px-3 py-1.5 rounded-lg transition-colors ${view === 'products' ? 'bg-slate-800 text-white' : 'text-slate-400 hover:text-white'}`}
            >
              Riskli Ürünler
            </button>
            <button 
              onClick={() => setView('about')} 
              className={`px-3 py-1.5 rounded-lg transition-colors ${view === 'about' ? 'bg-slate-800 text-white' : 'text-slate-400 hover:text-white'}`}
            >
              AI Aksiyonları
            </button>
          </nav>
      ...
      <main className="p-6 max-w-6xl mx-auto w-full flex-1">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl">
            ...
            <span className="text-slate-400 text-xs mt-2 block">Sektör Ortalaması: %8.5</span>
          </div>

          <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl">
            <span className="text-slate-400 text-xs font-semibold">Tahmini Kâr Kaybı</span>
            <div className="text-2xl font-bold text-white mt-1">₺18,450</div>
